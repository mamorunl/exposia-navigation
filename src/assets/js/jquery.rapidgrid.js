    ...
    $.rapidgrid = function (el, options) {
        ...
        rg.reset = function () {
            ...
            $('#canvas .rg-row').each(function() {
                var $existingrgcontrols = $(this).children('.rg-row-controls');
                $old_container = "no";
                if($existingrgcontrols.length) {
                    var $old_class = $existingrgcontrols.find('.class-row-input').first().val();
                    var $old_container = $existingrgcontrols.find('.btn.active').children('input').val();
                    $existingrgcontrols.remove();
                }
